...
import org.beangle.commons.lang.{Doubles, Numbers}
...
class DefaultCourseGradeCalculator extends CourseGradeCalculator {
  ...
  protected def reserve(project: Project, score: Float, state: CourseGradeState): Float = {
    Doubles.round(score.toDouble, if null == state then getDefaultScorePrecision(project) else state.scorePrecision).toFloat
  }
